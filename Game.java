...
public class Game {
   ...
   private void initializeGame() {
      ...
      // until each player has the appropriate number of cards 
       ...
       //loop through each player do this handSize times
      for (int i = 0; i < handSize; i++) {
         for (Player p : players) {
            p.getHand().addCard(drawPile.discard());
            nextPlayer();
         }
      }

      // Move top card from draw pile to the discard pike
   ...
   private Player nextPlayer() {
      ...
      if (bool == true) {
         ...
         String playerName = players[current].getName();
         displaySkippedPlayer(playerName);
         waitForUser();
         displayGameState();
      }
      
      //if end of array of players go back to first player (0 in array)
      if (current >= players.length) {
         current = 0;
      }
    
    //if the top card is a queen (rank 12) then skip this player.
    //recusrsively call nextPlayer() becatse the current++ will 
    // skip the player and then check to make sure it is not out of bounds or 
    // a queen again
      if (players[current].getHand().top().getRank() == 12) {
      ...
      //how do we implement the display skipped and the other thing?? ask 
    
    
      //if the card.rank is a queen then displaySkippedPlayer
  ...
  /**
   ...
   public static boolean cardMatches(Card card1, Card card2) {
      ...
      return false; //  if none of those are true it is false 
