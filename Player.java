...
public abstract class Player {
   ...
   public boolean hasPlayableCard(Card top) {
      ...
      //if any card in the hand matches the card on top then it is playable 
      //the cardMatches method checks this in Game class      
      
      for (int i = 0; i < this.hand.size(); i++) {
         Card c = hand.getCard(i);
         
         //if any of them match return true 
         if (Game.cardMatches(c, top)) {
            return true;
         }
      }
      
      return false;  //if it goes through the loop 
